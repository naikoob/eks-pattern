...
If the pod is for some reason directly attached to a load balancer, this is also the opportunity to unregister itself from the load balancer to stop sending new requests during the shutdown process (another reason to expose your application as a k8s service).
...
Similar to logging, there are multiple options for metrics. https://prometheus.io/docs/introduction/overview/[Prometheus] + https://grafana.com/[Grafana] is a popular open-source solutions. Amazon EKS Workshop has a guide on https://eksworkshop.com/intermediate/240_monitoring/[monitoring using Prometheus and Grafana on EKS]. There are also commercial solutions from Datadog, Dynatrace, New Relic, etc.
...
AWS based solution is https://aws.amazon.com/xray/[X-Ray], and a https://eksworkshop.com/intermediate/245_x-ray/[walk-through is available on EKS workshop].
...
a https://github.com/kubernetes-sigs/kubefed/blob/master/docs/userguide.md[user guide] is available if you want to try it out.

== Infrastructure as code

There are many infrastructure as code solutions, many have capability to interact with k8s. Take advantage of this capability to define both the cluster(s) and workloads to be deployed on the clusters as code.

This enables us to create/destroy clusters, along with the workloads, in a consistent manner, on demand. Potentially reduce running costs at the same time.

Depending on RTO, this can also be a very cost effective DR mechanism when running on the cloud (for compute). 
